...
class Space_Invaders_CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 32, 8, stride=4) #output will be 20x20 feature
        self.conv2 = nn.Conv2d(32, 64, 4, stride=2) #output will be 9x9
...
class Agent(object):
    ...
    def test_action(self,state, train):
        state = FloatTensor(state)
        if train:
            self.steps_done += 1
        # action will be selected based on the policy type : greedy or epsilon-greedy
        if self.eps_greedy:
            ...
            # explore or exploit?
            if random.random() > eps_threshold:
                return self.target(Variable(state, volatile=True).type(FloatTensor)).data.max(1)[1].view(1, 1)
                # with torch.no_grad():
                #    action = self.model(Variable(state))
                # return action.data.max(1)[1].view(1,1)
            else:
    ...
    def burn_memory(self):
        ...
        steps = 0
        state = np.zeros((3,84,84),dtype=np.float64)
        next_state = np.zeros((3,84,84),dtype=np.float64)
...
################################################################################################################################################
    ...
    def test_play(self,e, train=True):
        state_single = self.env.reset()
        # state_single = rgb2gray(resize(state_single,(84,84)))
        state_single = self.normalize_image(state_single)
        ...
        while True:
            # self.env.render()
            action = self.test_action([state], train)
            # print("action: ", action)
            next_state_single, reward, is_terminal, _ = self.env.step(action[0, 0])
            ...
            if is_terminal:
                if train:
                    ...
                    self.optimize_model()
                    # if steps %20 == 0:
                    print("Episode {} completed after {} steps till now | Total reward = {}".format(e + 1,
                                                                                                    self.steps_done,
                                                                                                    total_reward))
                self.episode_durations.append(steps)
                # self.plot_durations()
    ...
    def test(self,num_episodes):
        ...
        for e in range(num_episodes):
            rewards.append(self.test_play(e,train=False))
    ...
    def save_plots(self):
        ...
        plt.ylabel('Avg Reward')
        plt.plot(self.avg_rewards)
        plt.savefig(self.expt_name+'_test_avg_rewards_{}.png'.format(self.steps_done))
        ...
        plt.xlabel('Episode')
        plt.plot(self.train_rewards)
        plt.savefig(self.expt_name+'_train_rewards_'.format(self.steps_done))
    # def test(self,num_episodes):
    #     total_reward = 0
    #     print("-"*50)
    #     print("Testing for {} episodes".format(num_episodes))
...
def parse_arguments():
    ...
    parser.add_argument('--memory_burn_limit', type=int,default=25000, help='Till when to burn memory')
